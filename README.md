...
This registers a `SessionStart` and a `SessionEnd` hook in `.claude/settings.local.json`, enabling:
...
| **Bash for determinism only** | SHA-256 hashing, index building, link-graph analysis — correctness-critical operations only. |
...
| **Config is actually parseable** | Settings live in real frontmatter and are read by `config_get`, not just documented. |
...
│   ├── config.md                   User preferences (real YAML frontmatter)
│   ├── WIKI.md                     Instructions imported by your CLAUDE.md
│   ├── index.md                    ★ AUTO-GENERATED by build-index.sh ★
│   ├── log.md                      Append-only: what happened, when
...
└── hooks/                           Session lifecycle
    ├── session-start.sh             Wiki stats + PROACTIVE WIKI RULE (SessionStart)
    └── session-stop.sh              Stamp hot-cache for next session (SessionEnd)
```

Plus, at the repository root:

```
tests/run-tests.sh                   Behavioural test suite — no dependencies
